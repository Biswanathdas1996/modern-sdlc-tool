...
- **Knowledge Base (RAG System)**: Integrates with MongoDB Atlas for a RAG system with per-project data isolation. Each project gets its own MongoDB collections (`knowledge_chunks_{project_id}` and `knowledge_documents_{project_id}`) with dedicated indexes (`text_search_index`, `documentId_1`, `id_1`). Supports drag-and-drop document upload (TXT, MD, JSON, CSV), section-aware text chunking (1500-char chunks with 200-char overlap, breaking at headings/paragraphs/sentences), and TF-IDF semantic similarity search with keyword fallback during BRD and user story generation. Search uses `compute_corpus_similarity()` from `utils/embeddings.py` to build a unified TF-IDF vector space at query time. Re-ingestion API (`/reingest/{id}`, `/reingest-all`) allows updating existing documents with improved chunking. All KB operations (upload, search, list, delete, stats, reingest) require a `project_id` parameter.
