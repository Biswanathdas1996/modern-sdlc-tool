# Backend Architecture Diagram

## 🏗️ Layered Architecture

```
┌─────────────────────────────────────────────────────────────┐
│                     CLIENT (React/Vite)                      │
└──────────────────────────┬──────────────────────────────────┘
                           │ HTTP Requests
                           ▼
┌─────────────────────────────────────────────────────────────┐
│                  API LAYER (api/v1/)                         │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐   │
│  │ Projects │  │   BRD    │  │  Tests   │  │Knowledge │   │
│  │  Router  │  │  Router  │  │  Router  │  │   Base   │   │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘   │
│       │             │              │             │          │
│       └─────────────┴──────────────┴─────────────┘          │
│                           │                                  │
└───────────────────────────┼──────────────────────────────────┘
                            │ Delegates to
                            ▼
┌─────────────────────────────────────────────────────────────┐
│               SERVICE LAYER (services/)                      │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐   │
│  │    AI    │  │   JIRA   │  │Knowledge │  │  GitHub  │   │
│  │ Service  │  │ Service  │  │   Base   │  │ Service  │   │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘   │
│       │             │              │             │          │
│       └─────────────┴──────────────┴─────────────┘          │
│                           │                                  │
└───────────────────────────┼──────────────────────────────────┘
                            │ Uses
                            ▼
┌─────────────────────────────────────────────────────────────┐
│           REPOSITORY LAYER (repositories/)                   │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐   │
│  │ Project  │  │   BRD    │  │  Test    │  │  Story   │   │
│  │   Repo   │  │   Repo   │  │   Repo   │  │   Repo   │   │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘   │
│       │             │              │             │          │
│       └─────────────┴──────────────┴─────────────┘          │
│                           │                                  │
└───────────────────────────┼──────────────────────────────────┘
                            │ Persists to
                            ▼
┌─────────────────────────────────────────────────────────────┐
│              DATA LAYER (core/database.py)                   │
│                                                              │
│    ┌─────────────────┐          ┌──────────────────┐       │
│    │  In-Memory      │          │     MongoDB      │       │
│    │   Storage       │          │    (Knowledge    │       │
│    │  (Development)  │          │      Base)       │       │
│    └─────────────────┘          └──────────────────┘       │
└─────────────────────────────────────────────────────────────┘
```

## 🔄 Request Flow Example

```
1. Client Request
   │
   ├─→ POST /api/projects/analyze
   │   { "repoUrl": "https://github.com/user/repo" }
   │
2. API Router (api/v1/projects.py)
   │
   ├─→ Validates request using Pydantic schema
   ├─→ Calls background task
   │
3. Background Task
   │
   ├─→ ai.analyze_repository()
   │   │
   │   ├─→ GitHub Service fetches code
   │   ├─→ AI Service analyzes code
   │   └─→ Returns analysis data
   │
4. Repository Layer
   │
   ├─→ storage.create_analysis(data)
   ├─→ storage.update_project(id, updates)
   │
5. Response
   │
   └─→ Returns project with status "analyzing"
```

## 📦 Module Dependencies

```
app.py
├── core/
│   ├── config.py (settings)
│   ├── database.py (MongoDB)
│   └── logging.py (setup)
│
├── middleware/
│   └── logging.py (HTTP logging)
│
├── api/v1/
│   ├── projects.py
│   │   ├── schemas/
│   │   ├── repositories/
│   │   └── services/
│   │
│   └── knowledge_base.py
│       ├── schemas/
│       ├── repositories/
│       └── services/
│
├── services/
│   ├── ai_service.py
│   │   ├── core.config
│   │   ├── core.logging
│   │   └── utils.text
│   │
│   ├── jira_service.py
│   │   ├── core.config
│   │   └── core.logging
│   │
│   └── knowledge_base_service.py
│       ├── core.database
│       ├── core.logging
│       └── utils.text
│
├── repositories/
│   ├── base.py
│   ├── project_repository.py
│   │   └── base.py
│   │
│   └── storage.py
│       ├── base.py
│       └── project_repository.py
│
├── schemas/
│   ├── requests.py
│   └── entities.py
│
└── utils/
    ├── exceptions.py
    ├── response.py
    └── text.py
```

## 🎯 Separation of Concerns

| Layer | Responsibility | Example |
|-------|---------------|---------|
| **API** | HTTP handling, validation | Receive request, validate, return response |
| **Service** | Business logic, external APIs | Analyze repo, generate BRD, sync to JIRA |
| **Repository** | Data persistence | Save project, get analysis, update user story |
| **Schema** | Data structure, validation | Define request/response models |
| **Core** | Configuration, infrastructure | Settings, database, logging |
| **Utils** | Shared utilities | JSON parsing, text chunking, exceptions |

## 🔐 Configuration Flow

```
Environment Variables (.env)
         │
         ▼
    core/config.py
    (Pydantic Settings)
         │
         ├─→ app.py (FastAPI app)
         ├─→ services/ (AI, JIRA, etc.)
         └─→ core/database.py (MongoDB)
```

## 🚀 Startup Sequence

```
1. Load .env file
   ↓
2. Initialize Settings (core.config)
   ↓
3. Setup Logging (core.logging)
   ↓
4. Create FastAPI app
   ↓
5. Add Middleware (CORS, Logging)
   ↓
6. Register Routers (api/v1/*)
   ↓
7. Connect to MongoDB (if configured)
   ↓
8. Start Uvicorn server
   ↓
9. Ready to accept requests! ✅
```

## 📊 Data Flow

```
                    ┌─────────────────┐
                    │   Client App    │
                    └────────┬────────┘
                             │
                    ┌────────▼────────┐
                    │   API Router    │
                    │  (Validation)   │
                    └────────┬────────┘
                             │
            ┌────────────────┼────────────────┐
            │                │                │
    ┌───────▼──────┐  ┌──────▼──────┐  ┌────▼─────┐
    │  AI Service  │  │JIRA Service │  │KB Service│
    └───────┬──────┘  └──────┬──────┘  └────┬─────┘
            │                │                │
            └────────────────┼────────────────┘
                             │
                    ┌────────▼────────┐
                    │   Repository    │
                    │    (Storage)    │
                    └────────┬────────┘
                             │
                ┌────────────┴────────────┐
                │                         │
        ┌───────▼─────────┐     ┌────────▼────────┐
        │  In-Memory DB   │     │    MongoDB      │
        │  (Dev/Demo)     │     │  (Production)   │
        └─────────────────┘     └─────────────────┘
```

## 🎨 Design Patterns Used

1. **Repository Pattern**: Data access abstraction
2. **Service Layer Pattern**: Business logic separation
3. **Dependency Injection**: Loose coupling
4. **Factory Pattern**: Service creation
5. **Singleton Pattern**: Settings instance
6. **Strategy Pattern**: Different AI providers
7. **Middleware Pattern**: Request/response processing

## 🧪 Testing Strategy

```
Unit Tests
├── services/
│   ├── test_ai_service.py
│   ├── test_jira_service.py
│   └── test_kb_service.py
│
Integration Tests
├── api/v1/
│   ├── test_projects.py
│   ├── test_brd.py
│   └── test_knowledge_base.py
│
End-to-End Tests
└── test_workflows.py
    ├── test_analyze_and_generate_brd()
    ├── test_generate_user_stories()
    └── test_sync_to_jira()
```
# 1. Replace in-memory sessions with Redis
import redis
session_store = redis.Redis(host='redis-host', port=6379, db=0)

# 2. Add rate limiting
from slowapi import Limiter
limiter = Limiter(key_func=get_remote_address)

# 3. Use Celery/RQ for background tasks instead of threads
from celery import Celery
celery_app = Celery('tasks', broker='redis://localhost:6379/0')

# 4. Use async LLM calls consistently
result = await call_pwc_genai_async(prompt=prompt, ...)

# 5. Restrict CORS
cors_origins: list[str] = ["https://your-domain.com"]

# 6. Add deep health check
@app.get("/health")
async def health():
    db_ok = await check_db_connection()
    return {"status": "healthy" if db_ok else "degraded", "db": db_ok}