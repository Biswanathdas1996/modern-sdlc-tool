...
import { Bot, User, Send, Globe, RefreshCw, MessageSquare } from "lucide-react";
...
export default function WebTestAgentPage() {
  ...
  const chatMutation = useMutation({
    mutationFn: async (prompt: string) => {
      const response = await apiRequest("POST", "/api/v1/web-test-agent/chat", {
        prompt,
      ...
      <div className="flex-none p-6 pb-0">
        <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold" data-testid="text-page-title">Web Test Agent</h1>
              <p className="text-sm text-muted-foreground">
                AI-powered web application test case generation
              </p>
            </div>
          </div>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-4" data-testid="container-messages">
              {messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center py-12">
                  ...
                  <h3 className="text-lg font-medium mb-2">Web Test Generation</h3>
                  <p className="text-sm text-muted-foreground max-w-md">
                    Provide a URL and the agent will scrape the page, identify features,
                    and generate comprehensive test cases for web application testing.
                  </p>
                  <div className="mt-6 flex flex-wrap gap-2 justify-center">
                    {[
                      "Generate tests for https://example.com",
                      "What types of tests do you generate?",
                      "Test a login page flow",
                    ].map((suggestion) => (
                      <Button
                        key={suggestion}
                        variant="outline"
                  ...
                  {messages.map((message, index) => (
                    ...
                    >
                      <div className={cn(
                        ...
                        {message.role === "user" ? <User className="h-4 w-4" /> : <Globe className="h-4 w-4" />}
                      </div>
                  ...
                  {chatMutation.isPending && (
                    <div className="flex gap-3" data-testid="loading-indicator">
                      ...
                      <div className="flex items-center gap-2 rounded-lg bg-muted px-4 py-2">
                        ...
                        <span className="text-sm text-muted-foreground">Analyzing and generating tests...</span>
                      </div>
                    </div>
                  )}
            ...
            <div className="flex-none p-4 border-t">
              <form onSubmit={handleSubmit} className="flex gap-2">
                <Textarea
                  ...
                  placeholder="Enter a URL to test or ask about web testing... (Press Enter to send)"
